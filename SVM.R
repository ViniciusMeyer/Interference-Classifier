...
#train svm model
svm_model <- svm(category ~ ., data = total , type='C-classification',  nu=0.10, scale=TRUE, kernel="polynomial")

#svm percent function
...
svm_classifier_level<- function(a,b){
  ...
  for(i in 0:finish) {
    ...
    row <- data.frame(interval=as.numeric(i), resource="cpu", per=cpu)
    df <- bind_rows(df, row)
    row <- data.frame(interval=as.numeric(i), resource="mem", per=mem)
    df <- bind_rows(df, row)
    row <- data.frame(interval=as.numeric(i), resource="disk", per=disk)
    df <- bind_rows(df, row)
    row <- data.frame(interval=as.numeric(i), resource="net", per=net)
    df <- bind_rows(df, row)
    row <- data.frame(interval=as.numeric(i), resource="cache", per=cache)
    df <- bind_rows(df, row)
